...
export const CATALOG: LayerMetadata[] = [
  ...
  /* ------------------------------------------------------------ CONSERVATION -- */
  {
    ...
    spatialCoverage:
      "Global. 17,163 marine polygons in the August 2026 release; 6,728 shown on the map, which is 99.96% of the protected marine area",
    temporalCoverage: "Designations from the 1800s to present",
    latency: "Monthly release cycle",
    accessMethod:
      "Supplied shapefile, simplified to display geometry at build time and loaded only when the layer is switched on (1.8 MB compressed)",
    requiresProxy: false,
    requiredCredentials: [],
    license:
    ...
    caveats: [
      ...
      "Sites under 10 km² are omitted from the map — smaller than a pixel at the zooms this map supports. They are 61% of the records but 0.04% of the protected area, and remain in the source dataset.",
    ],
  },
  ...
  {
    id: "mur-sea-ice",
    title: "Sea-ice concentration (current)",
    plainSummary:
      "How densely packed the sea ice is, updated daily and available right up to the present.",
    whyItMatters:
      "This is the layer that answers 'what does the ice look like today?' — and, paired with the annual minimum and maximum dates, 'how does today compare with the same moment in past years?'",
    category: "ice",
    sourceType: "analysis",
    status: "live",
    cadence: "daily",
    sourceOrg: "NASA JPL",
    productName: "GHRSST L4 MUR Sea Ice Concentration",
    datasetId: "GHRSST_L4_MUR_Sea_Ice_Concentration",
    variables: [{ name: "sea ice concentration", unit: "%" }],
    spatialCoverage: "Global, both poles",
    spatialResolution: "1 km",
    temporalCoverage: "2002-06-01 to present",
    latency: "A few days",
    accessMethod: "NASA GIBS WMTS tiles, direct from browser",
    requiresProxy: false,
    requiredCredentials: [],
    license: "NASA open data",
    attribution: "Imagery courtesy of NASA GIBS. MUR produced by NASA/JPL.",
    sourceUrl: "https://podaac.jpl.nasa.gov/dataset/MUR-JPL-L4-GLOB-v4.1",
    caveats: [
      "Ice concentration here comes from the MUR analysis, which blends sources and fills gaps — it is not a single raw sensor reading.",
      "Web Mercator cannot represent the poles themselves, so the map cuts off around 85° latitude. For the true polar view, and for years before 2002, use the Arctic time machine.",
      "Passive-microwave ice retrievals are unreliable during summer surface melt, when meltwater ponds read as open water.",
    ],
  },
  {
    id: "ow-expedition-observations",
    title: "Ocean Warrior expedition observations",
    plainSummary:
      "Measurements taken from the vessel — temperature and salinity profiles, water samples, eDNA, and marine life observations.",
    whyItMatters:
      "This is the layer that does not exist anywhere else. Satellites see the surface and models fill the gaps; neither puts an instrument in the water at 80° north. Every other layer on this platform is context for this one.",
    category: "ocean_warrior",
    sourceType: "ocean_warrior_insitu",
    status: "planned",
    cadence: "near_real_time",
    sourceOrg: "Ocean Warrior",
    productName: "Expedition observation archive",
    variables: [
      { name: "temperature", unit: "°C", description: "CTD profile through the water column" },
      { name: "salinity", unit: "PSU" },
      { name: "depth", unit: "m" },
      { name: "eDNA occurrence", unit: "taxon" },
      { name: "marine mammal and seabird observations", unit: "count" },
    ],
    spatialCoverage: "Along expedition routes — remote and under-sampled ocean",
    spatialResolution: "Point measurements at stations",
    temporalCoverage: "From the first expedition onwards",
    latency: "Near-real-time from the vessel where bandwidth allows",
    accessMethod: "Ingested against the observation schema in this repository",
    requiresProxy: false,
    requiredCredentials: [],
    license: "To be confirmed with scientific partners",
    attribution: "Ocean Warrior and its scientific partners.",
    sourceUrl: "https://www.global-warrior.com/expeditions/ocean-warrior",
    caveats: [
      "COMING SOON. No real expedition measurements exist on this platform yet. The schema is built and validated so that data can be ingested without redesign, but nothing has been collected.",
      "When data does arrive it will carry a QC state — raw, provisional, passed, failed or expert-reviewed — and provisional values may change.",
      "An expedition measurement is a local sample at a moment in time. Its value comes from repeating the same transect year after year, not from any single reading.",
    ],
  },
  /* ------------------------------------------------------------ OCEAN WARRIOR -- */
  {
